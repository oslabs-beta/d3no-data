...
export default function PieChart(props:PieProps) { 
  ...
  function makePiechart (props:PieProps){
    ...
    const fakeData:{data:number, label:string} [] = [];
    ...
    //   fakeData.push({data : data, label: label})
    // }
    ...
    const arcs = g.selectAll('arc')
      .data(pie(Array.from(data,(data) => data.data)))
      .enter()
      .append("g")
      .attr("class", "arc")
    ...
    arcs.append("path")
      .attr("fill", function (d :number[] , i : number){
        return color(i/Array.from(data,(data) => data.data).length)
      })
      .attr("d", arc)
    ...
    arcs.append("text")
      ...
      .text( function(d : number, i : number) {return Array.from(data,(data) => data.label)[i]})
